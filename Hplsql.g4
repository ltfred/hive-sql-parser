...
expr_stmt :             // Standalone expression
       {!strings.EqualFold(p.GetTokenStream().LT(1).GetText(), "GO")}? expr
     ;
...
create_routine_params :
     ...
     | {!strings.EqualFold(p.GetTokenStream().LT(1).GetText(), "IS") &&
...
select_list_alias :
       {!strings.EqualFold(p.GetTokenStream().LT(1).GetText(), "INTO") && !strings.EqualFold(p.GetTokenStream().LT(1).GetText(), "FROM")}? T_AS? qident
...
from_alias_clause :
       {!strings.EqualFold(p.GetTokenStream().LT(1).GetText(), "EXEC") &&
...
delete_alias :
       {!strings.EqualFold(p.GetTokenStream().LT(1).GetText(), "ALL")}?
       T_AS? qident
     ;
...
func_param : 
       {!strings.EqualFold(p.GetTokenStream().LT(1).GetText(), "INTO")}? (ident T_EQUAL T_GREATER?)? expr
     ;     
